...
AppServices.Controllers.controller('PerformanceCtrl',
  [ 'data',
    'performance',
    'ug',
    '$scope',
    ...
    '$timeout',
    '$route',
    function (data, performance, ug, $scope, $routeParams, $location, $rootScope, $compile, $anchorScroll, $timeout, $route) {

      $scope.routeParams = $routeParams;
      $scope.location = $location;

      //turn it on when user gets here
      if(!$scope.autoUpdate){
      ...
      var getAppConfig = function(){
        return data.jsonp_simple('apigeeMobileConfig', '', {demoApp:$rootScope.demoData});
      }
...
//          }
      ...
      $rootScope.compare = function (reference,toggleUpdate) {
        if (reference === $rootScope.currentCompare) {
          ...
          if($rootScope.currentCompare !== 'NOW'){
            window.clearInterval($scope.$parent.timerUpdate);
            window.clearInterval($scope.$parent.autoUpdate);
            $scope.$parent.autoUpdate = null;
          }
        }
        $location.search('currentCompare', $rootScope.currentCompare);
      }
...
//      --------------------------------- Watching event and variables for page change
      ...
      $rootScope.$watch('currentApp',function(value,oldvalue){
        //force route reload on app change. this has to happen so everything can be re-inited
        if(value !== oldvalue){
          $route.reload();
        }
      })
     ...
     $scope.requestChartData = function(chartData,newFilter,callDesc,callback,runDataCheck){
       ...
       data.jsonp(chartData, newFilter, callDesc, function (mydata) {
         ...
         console.log('Problem getting initial chart data', fail);
       });
     }
     ...
     $scope.requestLogData = function(chartData,newFilter,callDesc,callback){
       data.jsonp(chartData, newFilter, callDesc, function (mydata) {
         ...
         console.log('Problem getting initial log data', fail);
       });
     }
...
//        $routeParams.networkChartFilter = '1'
        ...
        $scope.deferredLogin.promise.then(getAppConfig).then(function(appconfig){
          //set the appid
          setAppId(appconfig.instaOpsApplicationId)
        }).then(getChartCriteria).then(function(chartCriteriaList){
...
//                chartDataCall(deviceModelFilter, 'bar', $scope.ecCharts.deviceModels, 'convertParetoChart', 'bar1', function (chartData) {
                function convertData(forceCompare,srcdata){
                  if (mainChart.type === 'bar') {
                    $scope.perfCharts.requestTimes = performance[mainChart.call](srcdata.responseTimes, $scope.perfCharts.requestTimes, dataDescription[mainChart.dataDescriptions[0]], $rootScope.selectedtimefilter.settings, forceCompare);
                    $scope.perfCharts.requests = performance[mainChart.call](srcdata.requestErrorCounts, $scope.perfCharts.requests, dataDescription[mainChart.dataDescriptions[1]], $rootScope.selectedtimefilter.settings, forceCompare);
                  } else {
                    $scope.perfCharts.requestTimes = performance[mainChart.call](srcdata.chartData, $scope.perfCharts.requestTimes, dataDescription[mainChart.dataDescriptions[0]], $rootScope.selectedtimefilter.settings, forceCompare);
                    $scope.perfCharts.requests = performance[mainChart.call](srcdata.chartData, $scope.perfCharts.requests, dataDescription[mainChart.dataDescriptions[1]], $rootScope.selectedtimefilter.settings, forceCompare);
                  }
                }
      ...
      $scope.errorsCrashesSetup = function (ecDetail) {
        ...
        $scope.deferredLogin.promise.then(getAppConfig).
          then(function(appconfig){
            //set the appid
            setAppId(appconfig.instaOpsApplicationId)
          }).
          then(getChartCriteria).
                  ...
                  $scope.requestChartData(chartData,newFilter[chartFilterContext],
                    {
                      ...
                      $scope.ecCharts.totals = performance[mainChart.call](mycomparedata.chartData, $scope.ecCharts.totals, dataDescription[mainChart.dataDescriptionKey], $rootScope.selectedtimefilter.settings, $rootScope.currentCompare);
                    });
                }

                $scope.ecCharts.totals = performance[mainChart.call](mydata.chartData, $scope.ecCharts.totals, dataDescription[mainChart.dataDescriptionKey], $rootScope.selectedtimefilter.settings, 'NOW');
            ...
            $scope.requestChartData(chartData,newFilter[chartFilterContext],
              {
                ...
                console.log($rootScope.chartTemplate[mainChart.template])
                //make a copy of the blank template
                ...
                $scope.appUsageCharts.totals = performance[mainChart.call](mydata.chartData, $scope.appUsageCharts.totals, dataDescription[mainChart.dataDescriptions[0]], $rootScope.selectedtimefilter.settings, 'NOW');
                  ...
                  $scope.requestChartData(chartData,newFilter[chartFilterContext],
                    {
                      ...
                      $scope.appUsageCharts.totals = performance[mainChart.call](mycomparedata.chartData, $scope.appUsageCharts.totals, dataDescription[mainChart.dataDescriptions[0]], $rootScope.selectedtimefilter.settings, $rootScope.currentCompare);
                    });
                }
      ...
      function chartDataCall(chartData, newFilter, chartType, chartTemplate, convertMethod, dataDescriptionKey, successCallback) {
        data.jsonp(chartData, newFilter[chartFilterContext],
          ...
          function (mychartdata) {
            successCallback(performance[convertMethod](mychartdata.chartData, chartTemplate, dataDescription[dataDescriptionKey], $rootScope.selectedtimefilter.settings, newFilter.currentCompare), mychartdata);
          });
      }
